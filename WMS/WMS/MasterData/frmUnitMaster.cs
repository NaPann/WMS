...
namespace WMS.MasterData
{
    public partial class frmUnitMaster : Form
    {
        ...
        private void DGV()
        {
            NP_Cls.SqlSelect = "SELECT UnitCode, UnitName FROM m_Unit WHERE (FileStatus = N'1') ";
            if (!string.IsNullOrEmpty(this.txtSCode.Text.Trim()))
            {
                NP_Cls.SqlSelect += " AND (UnitCode LIKE '%" + this.txtSCode.Text.Trim() + "%') ";
            }
            if (!string.IsNullOrEmpty(this.txtSName.Text.Trim()))
            {
                NP_Cls.SqlSelect += " AND (UnitName LIKE '%" + this.txtSName.Text.Trim() + "%') ";
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (NP.ReqField(this.txtName, "Please enter Unit of Measure: !!") == false) { return; }
        ...
        private bool ChkDup(ref byte bRe)
        {
            ...
            {
                bRe = 0;
                NP_Cls.SqlSelect = "SELECT UnitCode FROM m_Unit WHERE (UnitCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '1')";
                ...
                else
                {
                    ...
                    NP_Cls.SqlSelect = "SELECT UnitCode FROM m_Unit WHERE (UnitCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '0')";
                    if (NP.GetClientDataSet(NP_Cls.SqlSelect).Tables[0].Rows.Count > 0)
                    {
                        if (NP.MSGB("This Code. is unused, Do you want to re-used this Code. ? ") == DialogResult.Yes)
                        {
                            NP_Cls.sqlUpdate = "UPDATE m_Unit SET filestatus = '1' WHERE (UnitCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "')"; string strErr = string.Empty;
        ...
        private void editToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                NP_Cls.SqlSelect = "SELECT * FROM m_Unit WHERE  (UnitCode = N'" + strSelect + "')";
                DataSet dsEdit = new DataSet(); dsEdit = NP.GetClientDataSet(NP_Cls.SqlSelect);
                if (dsEdit.Tables[0].Rows.Count > 0)
                {
                    this.txtCode.Text = dsEdit.Tables[0].Rows[0]["UnitCode"].ToString();
                    this.txtName.Text = dsEdit.Tables[0].Rows[0]["UnitName"].ToString();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (NP.MSGB("Do you want to Edit Data ?") == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    NP_Cls.sqlUpdate = "UPDATE    m_Unit" +
" SET              UnitName = @Name, Remark = @Remark WHERE (UnitCode = @Code)";
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (NP.MSGB("Do you want to delete ?") == DialogResult.Yes)
            {
                ...
                NP_Cls.SqlDel = "UPDATE m_Unit SET FileStatus = '0', UserChange = '" + NP_Cls.strUsr + "', DateChange = GETDATE()  WHERE (UnitCode = '" + this.dgvView[0, this.dgvView.CurrentRow.Index].Value.ToString() + "')";
        ...
        private void frmUnitMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            WMS.LoginSystem.frmMainMenu frm = (WMS.LoginSystem.frmMainMenu)Application.OpenForms["frmMainMenu"];
            frm.menuStrip1.Enabled = true;
        }
