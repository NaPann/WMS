...
{
    public partial class frmCurrencyMaster : Form
    {
        ...
        private void DGV()
        {
            NP_Cls.SqlSelect = "SELECT CurrencyCode, CurrencyName FROM m_Currency WHERE (FileStatus = N'1') ";
            if (!string.IsNullOrEmpty(this.txtSCode.Text.Trim()))
            {
                NP_Cls.SqlSelect += " AND (CurrencyCode LIKE '%" + this.txtSCode.Text.Trim() + "%') ";
            }
            if (!string.IsNullOrEmpty(this.txtSName.Text.Trim()))
            {
                NP_Cls.SqlSelect += " AND (CurrencyName LIKE '%" + this.txtSName.Text.Trim() + "%') ";
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (NP.ReqField(this.txtName, "Please enter Currency: !!") == false) { return; }
                ...
                if (NP.MSGB("Do you want to Add Data ?") == DialogResult.Yes)
                {
                    ...
                    try
                    {
                        ...
                        NP_Cls.SqlInsert = "INSERT INTO m_Currency (CurrencyCode, CurrencyName, Remark, UserCreate, DateCreate, FileStatus) VALUES (@Code,@Name,@Remark,@UC,getdate(),@St)";
        ...
        private bool ChkDup(ref byte bRe)
        {
            ...
            {
                bRe = 0;
                NP_Cls.SqlSelect = "SELECT CurrencyCode FROM m_Currency WHERE (CurrencyCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '1')";
                ...
                else
                {
                    ...
                    NP_Cls.SqlSelect = "SELECT CurrencyCode FROM m_Currency WHERE (CurrencyCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '0')";
                    if (NP.GetClientDataSet(NP_Cls.SqlSelect).Tables[0].Rows.Count > 0)
                    {
                        if (NP.MSGB("This Code. is unused, Do you want to re-used this Code. ? ") == DialogResult.Yes)
                        {
                            NP_Cls.sqlUpdate = "UPDATE m_Currency SET filestatus = '1' WHERE (CurrencyCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "')"; string strErr = string.Empty;
        ...
        private void editToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                NP_Cls.SqlSelect = "SELECT * FROM m_Currency WHERE (CurrencyCode ='" + strSelect + "')";
                DataSet dsEdit = new DataSet(); dsEdit = NP.GetClientDataSet(NP_Cls.SqlSelect);
                if (dsEdit.Tables[0].Rows.Count > 0)
                {
                    this.txtCode.Text = dsEdit.Tables[0].Rows[0]["CurrencyCode"].ToString();
                    this.txtName.Text = dsEdit.Tables[0].Rows[0]["CurrencyName"].ToString();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (NP.MSGB("Do you want to Edit Data ?") == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    NP_Cls.sqlUpdate = "UPDATE    m_Currency" +
" SET              CurrencyName = @Name, Remark = @Remark WHERE (CurrencyCode = @Code)";
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (NP.MSGB("Do you want to delete ?") == DialogResult.Yes)
            {
                ...
                NP_Cls.SqlDel = "UPDATE m_Currency SET FileStatus = '0', UserChange = '" + NP_Cls.strUsr + "', DateChange = GETDATE() WHERE (CurrencyCode = '" + this.dgvView[0, this.dgvView.CurrentRow.Index].Value.ToString() + "')";
        ...
        private void frmCurrencyMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            WMS.LoginSystem.frmMainMenu frm = (WMS.LoginSystem.frmMainMenu)Application.OpenForms["frmMainMenu"];
            frm.menuStrip1.Enabled = true;
        }
