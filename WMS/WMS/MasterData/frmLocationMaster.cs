...
namespace WMS.MasterData
{
    public partial class frmLocationMaster : Form
    {
        ...
        private void DGV()
        {
            ...
            {
                NP_Cls.SqlSelect += " AND (LocCode LIKE '%" + this.txtSCode.Text.Trim() + "%') ";
            }
            if (!string.IsNullOrEmpty(this.txtSName.Text.Trim()))
            {
                NP_Cls.SqlSelect += " AND (LocName LIKE '%" + this.txtSName.Text.Trim() + "%') ";
            }
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (NP.ReqField(this.txtName, "Please enter Location: !!") == false) { return; }
            ...
            if (NP.MSGB("Do you want to Add Data ?") == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    NP_Cls.SqlInsert = "INSERT INTO m_Location (LocCode, LocName, Remark, UserCreate, DateCreate, FileStatus) VALUES (@Code,@Name,@Remark,@UC,getdate(),@St)";
        ...
        private bool ChkDup(ref byte bRe)
        {
            try
            {
                NP_Cls.SqlSelect = "SELECT LocCode FROM m_Location WHERE (LocCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '1')";
                ...
                else
                {
                    ...
                    NP_Cls.SqlSelect = "SELECT LocCode FROM m_Location WHERE (LocCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "') AND (FileStatus = '0')";
                    if (NP.GetClientDataSet(NP_Cls.SqlSelect).Tables[0].Rows.Count > 0)
                    {
                        if (NP.MSGB("This Code. is unused, Do you want to re-used this Code. ? ") == DialogResult.Yes)
                        {
                            NP_Cls.sqlUpdate = "UPDATE m_Location SET filestatus = '1' WHERE (LocCode = '" + this.txtCode.Text.Trim().Replace("'", string.Empty) + "')"; string strErr = string.Empty;
        ...
        private void editToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                NP_Cls.SqlSelect = "SELECT * FROM m_Location WHERE (LocCode ='" + strSelect + "')";
                DataSet dsEdit = new DataSet(); dsEdit = NP.GetClientDataSet(NP_Cls.SqlSelect);
                if (dsEdit.Tables[0].Rows.Count > 0)
                {
                    this.txtCode.Text = dsEdit.Tables[0].Rows[0]["LocCode"].ToString();
                    this.txtName.Text = dsEdit.Tables[0].Rows[0]["LocName"].ToString();
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (NP.MSGB("Do you want to Edit Data ?") == DialogResult.Yes)
            {
                ...
                try
                {
                    ...
                    NP_Cls.sqlUpdate = "UPDATE    m_Location" +
" SET              LocName = @Name, Remark = @Remark WHERE (LocCode = @Code)";
        ...
        private void deleteToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (NP.MSGB("Do you want to delete ?") == DialogResult.Yes)
            {
                ...
                NP_Cls.SqlDel = "UPDATE m_Location SET FileStatus = '0', UserChange = '" + NP_Cls.strUsr + "', DateChange = GETDATE() WHERE (LocCode = '" + this.dgvView[0, this.dgvView.CurrentRow.Index].Value.ToString() + "')";
        ...
        private void frmLocationMaster_FormClosing(object sender, FormClosingEventArgs e)
        {
            WMS.LoginSystem.frmMainMenu frm = (WMS.LoginSystem.frmMainMenu)Application.OpenForms["frmMainMenu"];
            frm.menuStrip1.Enabled = true;
        }
